...
- 3초 로그 영상은 90일 보관한다. daily-reminders가 실행마다 `expire_log_videos`(서비스롤 전용, 30일 바닥,
  회당 상한)로 오래된 로그의 `video_path`를 먼저 비우고 그다음 파일을 지운다 — 파일 삭제가 실패해도 남는 건
  고아 파일이라 `media-gc`가 수거한다. 글·이모지·댓글과 로그 행은 남아 연속 기록이 끊기지 않는다.
