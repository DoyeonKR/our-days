...
- [x] **로그 retention**: ~~자동 삭제하지 않고 영구 보관~~ → **영상만 90일 보관**(2026-09-24 사용자 결정 —
  무료 저장 공간 1GB를 사진과 나눠 쓰면 1년 안팎에 찬다). 글·이모지·댓글은 영구. 정리는 daily-reminders가
  회당 100편, 켜는 스위치는 `migrations/20260924010000_log_video_retention.sql` 실행. 기록 허브와 월간 리캡에서 과거 기록으로 진입.
