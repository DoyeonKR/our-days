...
- [ ] **로그 retention**: 14일 브라우징 밖 로그/영상이 무기한 누적. (a) 14일 지나면 자동
  삭제(pg_cron+Edge) vs (b) 영구 보관 + '지난 기록' 진입점. — 제품 결정 필요.
- [ ] **고아 영상 서버 GC**: 게시 전 새로고침 시 드물게 남는 업로드본(개당 ~0.5MB).
  클라 정리는 구현됨 — 서버 배치는 Edge Function 필요(스토리지 API 삭제).
