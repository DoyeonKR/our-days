...
- **신화형(stage 5, 2026-08-11)**: 최종형 Lv.70 에서 **종을 초월한 영물 5종** —
  ...
  · ⚠ 구버전 **클라**가 신화 폼 문자열을 읽으면 petForm 폴백으로 '알'로 그린다 —
    상대가 옛 화면이면 앱 재실행 안내.
- **사신·천수·황룡(stage 6~8, 2026-08-31)** [사용자 요청 "신화형 다음 진화 단계, 그 윗단계까지 3단계 더"]:
  Lv.85 **사신** 🐉청룡/🦚주작/🐆백호/🐢현무 → Lv.100 **천수** 🔥봉황/⚖️해태 → Lv.120 **황룡** 🐲.
  갈래는 **위로 갈수록 좁아진다**(12→5→4→2→1) — 신화형 주석의 "12형→24형이면 컬렉션이 영영
  안 끝난다"의 반대 방향이다. 정점에 갈래가 없어야 '못 받은 폼'이 영구히 안 남는다.
  · **사신 방위는 섬의 네 축이 정한다** — 농사(청룡)·사냥(주작)·보글보글(백호)·꾸미기(현무).
    다른 놀이가 펫으로 흘러드는 통로를 넷으로 늘린 것(농사 ★5→진화가 이미 그런 통로였다).
  · ⚠ **여기만 고정 우선순위를 쓰면 안 된다.** 이 재료는 펫이 아니라 **섬**에 붙어 있어
    은퇴해도 안 줄어든다 — "농사 18+ 면 청룡"으로 두면 한 번 넘긴 계정은 **영원히 청룡만**
    나오고 나머지 셋이 죽는다. `divineDirection` 은 각 축을 기준으로 나눈 **비율**을 비교해
    다음 판에 갈아탈 수 있게 한다(ascend.test 가 이 재선택 가능성을 잠근다).
  · 천수는 은퇴 루프를 몇 번 돌았나로 갈린다(박물관 8종+ → 봉황=다시 태어나는 새, 그 외 해태).
    황룡은 조건 없음 — Lv.120 이면 누구나. 유대를 새 게이트에 안 쓴 건 **솔로 모드** 때문이다
    (사신 4축은 전부 혼자 된다).
  · ⚠ **`stage < 5` 하드코딩이 두 곳 있었다**(refreshEvolveFlag·evolutionPreview). 폼과 앵커만
    늘리고 그걸 놓치면 pendingEvolve 가 영영 안 서고 **에러도 안 난다**. 지금은 `MAX_STAGE`.
  · 앵커 확장([85,23500] [100,33500] [120,46000])은 신화형 때와 같이 **소급**된다 —
    Lv.70 상한에 걸린 채 쌓이던 careXp 가 배포 즉시 레벨이 된다(진화는 한 칸씩만 오른다).
  · ⏳ **아트는 아직 임시다** — `ASCEND_BORROW`(pixelart) / PET_ART 가 기존 골격을 빌려 쓴다.
    2차에서 전용 골격 3개(용·조·귀)로 교체하면 7종이 다 덮인다. 빌리기를 지우면 알로 보인다.
