...
//  · 2026-09-23 "신화 등급 윗 등급도 만들어 이미지까지 그리고"]
//
// 사다리의 모양은 **깔때기**다: 최종 20형 → 5 → 4 → 2 → 1. 갈래는 아래에서 벌어지고 위에서
...
test("★ 사다리는 8(황룡)에서 끝난다 — 위가 없다", () => {
  assert.equal(MAX_PET_STAGE, 8);
  assert.equal(PET_FORMS[APEX_FORMS[0]].stage, 8);
  assert.equal(nextEvolution(APEX_FORMS[0], 100, 10, 0, 3, undefined, {}), null, "황룡 위는 없다");
});
...
test("★ 깔때기 — 위로 갈수록 갈래가 좁아진다", () => {
  ...
  for (let st = 5; st <= MAX_PET_STAGE; st++) {
...
test("★ 신화형(5)에서 사신(6)으로 넘어간다", () => {
  const f = TUNING.pet.branch.s6Full;
  const asc: EvoContext = { huntBest: f.hunt * 2 };
  for (const from of ["tiger", "lion", "giraffe", "bengal_tiger", "mudeung_tiger"]) {
    assert.equal(nextEvolution(from, 90, 5, 0, 0, undefined, asc), "vermilion_bird", `${from} → 주작`);
  }
});
...
test("★ 천수 — 여러 생을 거쳤으면 봉황, 한 생이면 해태", () => {
  ...
  assert.equal(nextEvolution("azure_dragon", 90, 5, 0, 0, undefined, { museum: need }), "phoenix");
  assert.equal(nextEvolution("azure_dragon", 90, 5, 0, 0, undefined, { museum: need - 1 }), "haetae");
  assert.equal(nextEvolution("black_tortoise", 90, 5, 0, 0, undefined, {}), "haetae", "박물관 정보가 없으면 해태");
});

test("★ 황룡은 갈래가 없다 — 끝까지 간 사람은 모두 같은 곳에 닿는다", () => {
  for (const from of CELESTIAL_FORMS) {
    assert.equal(nextEvolution(from, 10, 0, 9, 0, undefined, {}), APEX_FORMS[0], `${from} → 황룡`);
    assert.equal(nextEvolution(from, 100, 10, 0, 3, undefined, {}), APEX_FORMS[0], "조건과 무관하게 황룡");
  }
});
...
test("★ evoContextOf 가 섬의 네 축과 박물관을 그대로 읽는다", () => {
  const s = fresh();
...
test("★ 컬렉션 진도에 새 단계가 포함된다", () => {
  const t = evolutionTree(fresh());
  assert.equal(t.finalsTotal, 20 + 5 + 4 + 2 + 1, "최종형+신화+사신+천수+황룡");
});
...
test("★ 새 폼이 알로 그려지지 않는다 — 픽셀·일러스트 둘 다 등록돼 있다", () => {
  ...
  for (const key of [...DIVINE_FORMS, ...CELESTIAL_FORMS, ...APEX_FORMS]) {
    // 정규식 대신 문자열 — 이스케이프를 한 겹 잃으면 '못 찾음'으로만 터져 원인이 안 보인다
    assert.ok(pixel.includes(`\n  ${key}: { frames:`), `pixelart.ts 의 MYTHICS 에 ${key} 가 없다`);
    assert.ok(/[A-Z]/.test(svg.split(`\n  ${key}: `)[1]?.[0] ?? ""), `pets.tsx 의 PET_ART 에 ${key} 가 없다`);
  }
});

// ── 아트 — "색만 다른 종" 금지 ──────────────────────────────────────
// 1차(2026-08-31 브랜치)는 기존 골격을 빌려서 주작·봉황·해태가 **같은 사자**로, 청룡·황룡이
// **같은 기린**으로 보였다 — "생긴건 다 똑같고 눈동자하고 피부만 다르잖아"로 이미 퇴짜난 상태.
// 그래서 눈이 아니라 **실루엣(잉크 칸 집합)** 을 잰다. 팔레트만 다른 두 폼(호랑이↔뱅갈)은
// 오라 글리프 차이뿐이라 12칸 안팎이다 — 기준선은 그보다 확실히 큰 30칸.
const SILHOUETTE_MIN = 30;
function ink(form: string): Set<number> {
  const s = petSprites(form)[0];
  const out = new Set<number>();
  s.rows.forEach((r, y) => [...r].forEach((ch, x) => { if (ch !== ".") out.add(y * 48 + x); }));
  return out;
}
function silhouetteDiff(a: string, b: string): number {
...
test("★★ 새 층의 폼은 아래층 어느 폼과도 실루엣이 다르다", () => {
  const upper = [...DIVINE_FORMS, ...CELESTIAL_FORMS, ...APEX_FORMS];
  const lower = Object.values(PET_FORMS).filter((f) => f.stage <= 5).map((f) => f.key);
  for (const u of upper) {
    for (const l of lower) {
      const d = silhouetteDiff(u, l);
      assert.ok(d >= SILHOUETTE_MIN, `${u} 가 ${l} 와 실루엣이 ${d}칸만 다르다 — 색만 바꾼 것처럼 보인다`);
    }
  }
});

test("★★ 골격을 나눠 쓰는 새 폼끼리도 실루엣이 갈린다", () => {
  // 청룡·황룡은 용 골격, 주작·봉황은 새 골격을 같이 쓴다 — 여의주·꼬리깃 셋이 그 차이다.
  const upper = [...DIVINE_FORMS, ...CELESTIAL_FORMS, ...APEX_FORMS];
  for (let i = 0; i < upper.length; i++) {
    for (let j = i + 1; j < upper.length; j++) {
      const d = silhouetteDiff(upper[i], upper[j]);
      assert.ok(d >= SILHOUETTE_MIN, `${upper[i]} ↔ ${upper[j]} 실루엣 차이 ${d}칸`);
    }
  }
});

test("★ 백호·해태는 자기 원형(뱅갈·사자)과 확실히 갈린다", () => {
  // 둘은 호랑이·사자가 맞는 종이라 골격을 공유한다. 팔레트만 바꿔서는 안 된다는 뜻을 따로 잠근다.
  assert.ok(silhouetteDiff("white_tiger", "bengal_tiger") >= 100, "백호가 뱅갈과 너무 닮았다(불꽃 날개·구름)");
  assert.ok(silhouetteDiff("haetae", "lion") >= SILHOUETTE_MIN, "해태가 사자와 너무 닮았다(외뿔·방울)");
});
