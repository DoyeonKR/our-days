...
test("신형 히어로는 원본만 렌더링하고 종별 모션을 낸다", () => {
  ...
  for (const removedEyeLayer of ["hero-closed-eyes", "hero-eye-shut", "hero-v2-blink", "EyeProfile", "can-blink"]) {
    assert.ok(!hero.includes(removedEyeLayer), `${removedEyeLayer} 눈 레이어가 남아 있다`);
    assert.ok(!css.includes(removedEyeLayer), `${removedEyeLayer} 눈 스타일이 남아 있다`);
  }
  ...
  for (const motion of ["hero-v2-bird", "hero-v2-canine", "hero-v2-feline", "hero-v2-tall", "hero-v2-ear-twitch"]) {
