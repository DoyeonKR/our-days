...
test("탭 타겟은 보이는 크기가 아니라 가짜 요소로 넓힌다 (44px 기준)", () => {
  ...
  // ── 일부러 남긴 예외 두 곳 (배포본 실측 후 판단). 둘 다 WCAG 2.5.8(AA, 24px)은 넘는다.
  //  · 캘린더 날짜 칸 40~42×54 — 375px 에 7칸이라 폭이 물리적으로 안 나온다.
  //    ::after 를 46 으로 키워도 옆 칸이 경계에서 주인을 가져간다(늘려도 실측이 안 오른다).
  //  · 사진 오버레이의 수정/삭제 32×46 — 둘이 붙어 있어 서로의 확장을 뺏는다.
  //    폭을 44 로 키우면 좁은 기기(320px)에서 셀 밖으로 나간다. 날씨 앱에서 음수 마진으로
  //    자리를 만들다 레이아웃을 깬 적이 있어, 여기선 안 건드리고 기록만 남긴다.
});
