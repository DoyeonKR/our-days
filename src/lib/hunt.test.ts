...
test("★ 오프라인 창은 끝점(now) 기준 — 한도 소진일에 떠나 며칠 비워도 복귀 몫이 있다 [리뷰 2026-08-25]", () => {
  /* 자정 분할 1차 구현이 창을 떠난 시점(s0.at)에 앵커해서, 며칠 부재의 오프라인 10시간이
     통째로 '떠난 날'에 귀속됐다 — 그날 한도를 다 받고 떠났으면 수백 마리를 잡고도 +0💗.
     최근 10시간(now 기준)을 날짜별로 귀속해야 복귀일의 신선한 한도가 산다. */
  const drained = settle(createHunt(TM), TM + 12 * 3600 * SEC, 999, 99, false); // 정오까지 한도 소진
  assert.ok(drained.gain.dayCapped, "전제 실패: 한도에 안 닿았다");
  const back = settle(drained.hunt, TM + 12 * 3600 * SEC + 3 * day, 999, 99, true); // 3일 뒤 복귀
  assert.ok(back.gain.kills > 0, "전제 실패: 부재 정산에 처치가 없다");
  assert.ok(back.gain.coins > 0, "장기 부재가 +0💗 — 오프라인 창이 떠난 날에 통째로 귀속됐다");
});
